...
\textbf{(G0) Allow users to signal traffic violations}
\textbf{(G1) Allow users to include pictures in violation reports}
\textbf{(G4) The system must be able to retrieve the geographical position of a violation and add it to the report}
\textbf{(G3) The system must recognise (from pictures) license plates}
\textbf{(G5) The system must store user made reports}
\textbf{(G2) Allow users and authorities to mine information collected by the application}
\textbf{(GA1.1) The system must be able to cross the data collected with information about the accidents coming from the municipality.} 
\textbf{(GA1.2) The system must be able to suggest possible interventions to decrease the risk of violations in unsafe areas.}
\textbf{(GA2.1) Allow the local police to retrieve data about violation to generate traffic tickets}
\textbf{(GA2.2) The system must be able to ensure the veracity of the information sent by the users}
\textbf{(GA2.3) The system must track whether the police has taken care of a certain violation}
\textbf{(GA2.4) Traffic tickets must be issued to the person that owns the vehicle that committed the violation}
\textbf{(GA2.5) The system must be able to build statistics from the information about issued tickets} 
