...
\item \textbf{The system must be able to build statistics from the information about issued tickets}
	...
	\item \textbf{User Registration}
		...
		Exceptions        		&       	
				\begin{enumerate}[label=(\alph*)]
					...
					\item The user is already registered. An error message asking to reinsert the data is displayed.
					\item Mandatory fields are left empty or filled with invalid entries. An error message asking to reinsert the data is displayed.
					\item The user does not accept privacy policy. An error message asking to accept the policy is displayed.
	...
	\item \textbf{Institution Registration}
		...
		Event Flow        		&          
				\begin{enumerate}[label=(\alph*)]
					...
					\item In the website institution dedicated area, the authority finds a mandatory registration form. Among the data must be provided a certificate for institution acknowledgement
		...
		Exceptions        		&
				...
				\end{enumerate}
				All exceptions raise an error asking to complete the remaining operations \\ \hline
	\end{tabular}
	\end{table}
	...
	\item \textbf{Violation Signaling}
		...
		Event Flow        		&          
				\begin{enumerate}[label=(\alph*)]
					...
					\item The app asks the user to attach a photo of the violation, picturing the vehicle involved with license visible, and to manually insert license plate number of the vehicle.
					\item The user takes a photo of the violation within SS app and insert license plate number of the vehicle.
					\item The app localy verifies if license plate pictured and manually inserted match and readies the violation report. 
					\item The app shows a menu displaying a " Send Report" and a "Cancell" options.
					\item The user tap on "Send Report" to send the violation report to SS systems.
					\end{enumerate}\\ \hline
		Output Conditions &    		SS mobile client sends the report to SS server. The report can now be stored in SS DB.\\ \hline
		Exceptions        		&       	
				\begin{enumerate}[label=(\alph*)]
					\item License plate number recognized by the app and manually inserted do not match. In this case, an error message is raised and SS asks the user to retake the photo.
				\end{enumerate}\\ \hline
	\end{tabular}
	\end{table}
	...
	\item \textbf{Providing Violations Information to Users}
		\begin{table}[h!]
		\begin{tabular}{|l|p{12,5cm}|}
		\hline
		Actors            			&       	User\\ \hline
		...
		Event Flow        		&          
				\begin{enumerate}[label=(\alph*)]
					\item The user opens SS app and taps on the "Violations Map" option
					\item
					\item
					\end{enumerate}\\ \hline
